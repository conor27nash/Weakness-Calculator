...
func TestCalculateAttacking(t *testing.T) {
	ctx := context.Background()
	...
	tests := []struct {
		...
		{name: "fire is strong against steel", attackType: "fire", checkType: "steel", wantMult: 2.0},
		{name: "fire is strong against grass", attackType: "fire", checkType: "grass", wantMult: 2.0},
		{name: "fire is weak against water", attackType: "fire", checkType: "water", wantMult: 0.5},
		{name: "fire is neutral against fighting", attackType: "fire", checkType: "fighting", wantMult: 1.0},
		{name: "dragon has no effect on fairy", attackType: "dragon", checkType: "fairy", wantMult: 0.0},
		{name: "ghost has no effect on normal", attackType: "ghost", checkType: "normal", wantMult: 0.0},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			results, err := backend.CalculateAttacking(ctx, tt.attackType)
			if err != nil {
				t.Fatalf("unexpected error: %v", err)
			}
			...
			for _, r := range results {
				if r.AttackType == tt.checkType {
					if r.Multiplier != tt.wantMult {
						t.Errorf("CalculateAttacking(%s)[%s] = %v, want %v",
							tt.attackType, tt.checkType, r.Multiplier, tt.wantMult)
					}
					return
				}
			}
			t.Errorf("defender type %s not found in results", tt.checkType)
		})
	}
}

func TestCalculateAttacking_Returns18Results(t *testing.T) {
	ctx := context.Background()

	results, err := backend.CalculateAttacking(ctx, "fire")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(results) != 18 {
		t.Errorf("CalculateAttacking returned %d results, want 18", len(results))
	}
}

func TestCalculateAttacking_CancelledContext(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	cancel()

	_, err := backend.CalculateAttacking(ctx, "fire")
	if err == nil {
		t.Error("expected error from cancelled context, got nil")
	}
}
